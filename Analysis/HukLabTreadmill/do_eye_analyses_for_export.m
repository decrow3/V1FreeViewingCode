function [EyeStat, Export]=do_eye_analyses_for_export(D,opts)
% Also collect all binned data for modelling
...
invalid_trials=((sum(eyeFlag'==4))>(nbins/2))|((sum(isnan(runSpeed)'))>(nbins/2));
trialfilter=find(~invalid_trials);

runSpeed(invalid_trials,:) = nan;
GratPhase(invalid_trials,:) = nan;
PupilArea(invalid_trials,:) = nan;
eyeFlag(invalid_trials,:) = nan;
eyePosX(invalid_trials,:) = nan;
eyePosY(invalid_trials,:) = nan;
eyeSessSet(invalid_trials,:) = nan;

%%

% runSpd = behav{1}(goodIx,:);
spd = nanmean(runSpeed,2);
...
%% All on same 'clock' binned to trials, ntrials x cells
Export.Robs = Robs(trialfilter,:);
Export.RunSpeed = spd(trialfilter,:);
Export.SacRate=SacHz(trialfilter,:);
Export.SacMag=MSac(trialfilter,:);
Export.PupilArea=eyeSize(trialfilter,:);
Export.Eyevar=varXY(trialfilter,:);
Export.StimDir=StimDir(trialfilter,:);
Export.StimSF=StimFreq(trialfilter,:);
Export.StimTF=StimSpeed(trialfilter,:).*StimFreq(trialfilter,:);
