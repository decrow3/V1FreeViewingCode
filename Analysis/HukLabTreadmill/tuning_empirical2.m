function stat = tuning_empirical2(D, varargin)
...
for cc = 1:NC
    ...
    %on and offsets are already in seconds
...
for cc = 1:NC
    ...
    sf = D.GratingFrequency(iix);
    tf = D.GratingSpeeds(iix);

    tix = lags > win(1) & lags < win(2);
    R = sum(spk(:,tix),2);

    fitS(cc) = tcurve.fit_tuningcurve_sf_tf(th, sf, tf, R, false);
    residualS_after_fitS(cc).res=R-fitS(cc).tuningFun(th,sf,tf);
    residualS_after_fitS(cc).iix=iix;
    residualS_after_fitS(cc).spk=spk;
    residualS_after_fitS(cc).th=th;
    residualS_after_fitS(cc).sf=sf;
    residualS_after_fitS(cc).tf=tf;
    residualS_after_fitS(cc).R=R;
    residualS_after_fitS(cc).rsquared=rsquared(R, fitS(cc).tuningFun(th,sf,tf), false, mean(R)); %compute explained variance
    ...
    fitR(cc) = tcurve.fit_tuningcurve_sf_tf(th, sf, tf, R, false);

    
    residualR_after_fitS(cc).res=R-fitS(cc).tuningFun(th,sf,tf);
    residualR_after_fitS(cc).gain=(R-fitS(cc).tuningFun(th,sf,tf))./fitS(cc).tuningFun(th,sf,tf);
    residualR_after_fitS(cc).iix=iix;
    residualR_after_fitS(cc).spk=spk;
    residualR_after_fitS(cc).th=th;
    residualR_after_fitS(cc).sf=sf;
    residualR_after_fitS(cc).tf=tf;
    residualR_after_fitS(cc).R=R;
    residualS_after_fitS(cc).rsquared=rsquared(R, fitS(cc).tuningFun(th,sf,tf), false, mean(R)); %compute explained variance
...
stat.TCfitS = fitS;
stat.TCfitR = fitR;
stat.residualR_after_fitS=residualR_after_fitS;
stat.residualS_after_fitS=residualS_after_fitS;
