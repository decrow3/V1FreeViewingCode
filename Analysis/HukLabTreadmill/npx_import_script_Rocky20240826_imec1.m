...
%%
if isempty(dir('/home/huklab/Documents/NPX_pilot/Output/RockyV2V1_20240826/'))
    mkdir('/home/huklab/Documents/NPX_pilot/Output/RockyV2V1_20240826/')
end
 save('/home/huklab/Documents/NPX_pilot/Output/RockyV2V1_20240826/Rocky20240826_V2_g0_t0.nidq_imecAp.mat','riseSent','riseRec','imecSent','imecSync','Event')
% clear Tout
 %load('/home/huklab/Documents/NPX_pilot/Output/RockyV2V1_20240826/Rocky20240826_V2_g0_t0.nidq_imecAp.mat')

 %% Missing signals
 length(riseRec)-length(riseSent)
[C,IA,IC] = unique(diff(riseRec),'stable'); %discont at IA
riseRecbreaks=IA(3:4);
nPrebreakRec=riseRecbreaks(1);
nPostbreakRec=length(riseRec)-riseRecbreaks(2); %count from end

[C,IA,IC] = unique(diff(riseSent),'stable'); %discont at IA
riseSentbreak=IA(3); % this is the diff between to samples, starting at IA
nPrebreakSent=riseSentbreak(1);
nPostbreakSent=length(riseSent)-riseSentbreak(1); %count from end

%Since recorded is more than (saved) sent
riseRec2=[riseRec(1:nPrebreakSent); riseRec(length(riseRec)-nPostbreakSent+1:end)];
%%
riseRec=riseRec2;
...
%save(['/home/huklab/Documents/NPX_pilot/Output/RockyV2V1_20240826/' 'Rocky20240826_V2_t0.nidq' 'dp_ev_ts2.mat'],'Event')
load(['/home/huklab/Documents/NPX_pilot/Output/RockyV2V1_20240826/' 'Rocky20240826_V2_t0.nidq' 'dp_ev_ts2.mat'],'Event')
...
StimPath='/mnt/NPX/Rocky/20240826/Rocky20240826/';%/mnt/NPX/Rocky/20240826/Rocky20240826/';
...
% Load facecall data
% dataPath1 ='/mnt/NPX/Rocky/20240826/Rocky 2024-07-04/facecal';
...
EyeExp=Exp;
EyeExp.D=Exp.D(caltrials(385:end));
%%
...
C.cmat= [   1.2073    0.9056   -6.9989   -0.5957   1.0444];
...
% cmat=[  5.9684    6.4481   -4.6499  -36.8079  -71.8362];
% C.cmat=cmat
 %%
 clear C
C = calibGUI(Exp,'use_smo',0);
C.cmat=[ 3.2443    3.3847   -7.3412   18.3972 -81.1366];
...
%% Load spikes
% dataPath = '/home/huklab/Documents/SpikeSorting/Output/Rocky/20240826/imec0_probe1_v2';
dataPath = '/home/huklab/Documents/RyanSorting/SpikeSortingTools/pipeline_results_Rocky20240826_V1V2_g0_imec1_locar_40_75/cur/cur_sorter_output/';
...
% dataPath = '/mnt/NPX/Rocky/20240826/catgt_Rocky20240826_V2MT_g0/Rocky20240826_V2MT_g0_imec1/kilosort4/';
...
% %% Retry with handful
% %% Load spikes
% dataPath = '/home/huklab/Documents/SpikeSorting/Output/2024-07-04/Retry';
% Exp.osp = load(fullfile(dataPath, 'spkilo.mat'));
% Exp.osp.st = Exp.osp.st;
% Exp.osp.st(Exp.osp.st<0)=0;
% % move zero to the end
...
% 
% Exp=load('/home/huklab/Documents/NPX_pilot/Rock20240826_V2MT_onlinedpi.mat');
% resync=load('/home/huklab/Documents/NPX_pilot/Rocky20240826_resync.mat');
% Exp.ptb2Ephys=resync.fun;
% Exp.FileTag='Rocky20240826_V2MT_resorted';
% %% Load spikes
% dataPath = '/home/huklab/Documents/SpikeSorting/Output/2024-07-04_FullyResorted/Rocky_2024-07-04-prb1/';
% Exp.osp = load(fullfile(dataPath, 'spkilo.mat'));
% Exp.osp.st = Exp.osp.st;
% Exp.osp.st(Exp.osp.st<0)=0;
% % move zero to the end
...
% 
% %% Second probe
% dataPath = '/home/huklab/Documents/SpikeSorting/Output/2024-07-04_FullyResorted/Rocky_2024-07-04-prb2/';
% Exp.osp2 = load(fullfile(dataPath, 'spkilo.mat'));
% Exp.osp2.st = Exp.osp2.st;
% Exp.osp2.st(Exp.osp2.st<0)=0;
% % move zero to the end
...
for fig=1:nfigs
    figure(fig)
    set(gcf,'Position',[0 0 1080 1080])
saveas(figure(fig),['/home/huklab/Documents/NPX_pilot/Output/RockyV2V1_20240826/Rocky20240826_V2V1_dotRFs/Rocky20240826_V2V1_dotRFs_' num2str(nperfig*(fig-1)+1) '_' num2str(nperfig*fig) '.pdf'])
end
