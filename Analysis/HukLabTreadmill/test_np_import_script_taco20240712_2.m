...
load('/home/huklab/Documents/MGS/taco20240712memorySaccades.memorySaccadeTraining1510.PDS','-mat')
PTBts_numbers=cellfun(@(X) X.datapixx.unique_number_time(:,1), PDS.data,'uniformOutput',false);
PTBword_numbers=cellfun(@(X) X.unique_number,PDS.data,'UniformOutput',false);

for ii=1:length(PTBword_numbers)
    for jj=1:6
        PTBword_numbers{ii}(jj)=mod(PTBword_numbers{ii}(jj), 2^8);
    end
end
ptbwords_numbers=cell2mat(PTBword_numbers);
ptbt_numbers=cell2mat(PTBts_numbers')';

%% Add in trial starts and stops
PTBts_starts=cellfun(@(X) X.timing.datapixxTRIALSTART(:,1), PDS.data,'uniformOutput',false);
PTBword_starts=cellfun(@(X) X.trialnumber,PDS.data,'UniformOutput',false);
for ii=1:length(PTBword_starts)
    for jj=1:1
        PTBword_starts{ii}(jj)=mod(PTBword_starts{ii}(jj), 2^8);
    end
end
ptbwords_starts=cell2mat(PTBword_starts);
ptbt_starts=cell2mat(PTBts_starts')';

PTBts_ends=cellfun(@(X) X.timing.datapixxTRIALEND(:,1), PDS.data,'uniformOutput',false);
PTBword_ends=cellfun(@(X) X.trialnumber,PDS.data,'UniformOutput',false);
for ii=1:length(PTBword_ends)
    for jj=1:1
        PTBword_ends{ii}(jj)=mod(PTBword_ends{ii}(jj), 2^8);
    end
end
ptbwords_ends=cell2mat(PTBword_ends);
ptbt_ends=cell2mat(PTBts_ends')';
%% combine
ptbwords=[ptbwords_numbers ptbwords_starts ptbwords_ends];
ptbt=[ptbt_numbers ptbt_starts ptbt_ends];

[ptbt,Is] = sort(ptbt);
ptbwords=ptbwords(Is);
%%
...
sentbits=dec2bin(ptbwords);
%sentbits(:,end-1)='0';
plot(ptbt_,bin2dec(sentbits(:,[2 3 4 5 6 7 8])),'.')
title('Unique signals sent by PTB')
%ylim([0 80]);
%xlim([ptbtime0 ptbtime1]); ylim([0 64]);
%    STOPPED HERE, SIGNALS STILL AREN'T RIGHT
%% if nec, update
Recbits=(switchbits(:,[1 2 3 4 5 6 7]));
RecWords=bin2dec(Recbits);

ptbwords=bin2dec(sentbits(:,[2 3 4 5 6 7 8]));
%%
% %% 
% strangehighs=(RecWords==62|RecWords==63);
% RecWords=RecWords(~strangehighs);
...
% hold off
% 
%% check (very manual), this just gets it close for syncing
% clf


offset=2071.8-1720822263;
...
for ll=1:1
    ...
    plot([ptbtimeend ptbtimeend],[0 112])
    hold off
    %xlim([x1-offset x2-offset]); ylim([0 112]);
    pause(0.2)
end
% 
% 
