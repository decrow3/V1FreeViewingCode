...
timingfilename='/mnt/NPX/Rocky/20230620/Rocky-v2medial-0620_g0/Rocky-v2medial-0620_g0_t0.nidq.bin';
nchannels=4;
samples=[];
dataformat=[];
memorymap=1;
...
spikefilename='/mnt/NPX/Rocky/20230620/Rocky-v2medial-0620_g0/Rocky-v2medial-0620_g0_imec0/Rocky-v2medial-0620_g0_t0.imec0.ap.bin';
nchannels=385;
samples=[];
dataformat=[];
memorymap=1;
...
%%
%save('/home/huklab/Documents/SpikeSorting/Output/2023-06-20/Rocky-0620_g0_t0.nidq_imecAp.mat','riseSent','riseRec','imecSent','imecSync')
load('/home/huklab/Documents/SpikeSorting/Output/2023-06-20/Rocky-0620_g0_t0.nidq_imecAp.mat')
%%
%Missed one?
riseSent(end-1:end)/30000-(riseRec(end-1:end)/30003.000300);

Event.ClosestAPSamples = interp1(riseRec,riseSent(1:end-1),Event.samplenumber);
Event.Ap_ts=Event.ClosestAPSamples/30000.076569037657; % from AP metafile

%risephD=find(diff(phDiode)>0); %in samples on nidaq
Event.PhDiodeClosestAPSamples = interp1(riseRec,riseSent(1:end-1),risePD);
...
%save(['/home/huklab/Documents/SpikeSorting/Output/2023-06-20/' 'Rocky-0620_g0_t0_2.nidq' 'dp_ev_ts.mat'],'Event')
% 
% close all
% clear Tout NiDaQout
%%

%/mnt/NPX/Rocky/20230620/Rocky-v2medial-0620_g0/Rocky-v2medial-0620_g0_t0.nidq.bin'
load(['/home/huklab/Documents/SpikeSorting/Output/2023-06-20/' 'Rocky-0620_g0_t0.nidq' 'dp_ev_ts.mat'],'Event')
...
StimPath='/mnt/NPX/Rocky/20230620/Rocky 2023-06-20/';
...
%% Run Fixbiter.m
toffset=-1687305770.174641132354736;
...
% check
x1=100; x2=1000;
%x1=3674.23; x2=3674.235;
for ll=1:10
figure(2)
hold off

scatter(RecWords_ts,RecWords,'.')
xlim([x1 x2]); ylim([0 60]);
pause(0.05)
hold on
...
%%
C.cmat=[1.0141    1.0394   -2.9995   -1.3924    1.0219];

%DPI:
%C.cmat=[0.6666    0.6931   -8.9993  -44.3209   15.1000];
cmat=C.cmat;
...
%% Load spikes
dataPath = '/home/huklab/Documents/SpikeSorting/Output/2023-06-20/';
...
%% plot subset
subset = [702 723 731 739 749 750 757 761 762 785 790 794 866 868 873 896 897 900 904 908 914 919 ...
    924 925 935 1242 1251 1254 1256 1266 1278 1285 1300 1305 1308 1309 1324 1454 1457 1510];
nsub = numel(subset);
