function [X, opts] = build_design_matrix_sf_tf(Stim, opts)
...
defopts.stim_dur = 1; 
defopts.tf_ctrs = [0.5 2 8];
defopts.sf_ctrs = [0.5 2 8];
...
tf_freq = Stim.speed_grating(:) .* stim_on; %TFs are saved in speed for historical reasons
...
if opts.use_tf_tents
    % temporal frequency (with basis)
    tf = tent_basis(tf_freq, opts.tf_ctrs) .* stim_on;
    opts.ntf = numel(opts.tf_ctrs);
else
    % temporal frequency (no tents)
    opts.tf_ctrs = unique(tf_freq(stim_on));
    opts.ntf = numel(opts.tf_ctrs);
    tf = (freq == opts.tf_ctrs(:)') .* stim_on;
end
...
Xbig = zeros(NT, opts.nd, opts.nsf, opts.ntf, opts.nphi);
for idir = 1:opts.nd
    for isf = 1:opts.nsf
        if opts.ntf > 1
            for itf = 1:opts.ntf
                if opts.nphi > 1
                    Xbig(:,idir, isf, itf, :) = xdir(:,idir).*sf(:,isf).*tf(:,itf).*xphase;
                else
                    Xbig(:,idir, isf, itf, 1) = xdir(:,idir).*sf(:,isf).*tf(:,itf);
                end
            end
        else
