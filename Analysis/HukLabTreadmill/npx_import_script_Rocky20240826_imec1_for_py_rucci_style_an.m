...
%% 
ephysPath='/mnt/NPX/Rocky/20240826/Ephys/Rocky20240826_V2MT_g0';
%These were unfortunately mislabeled
ephysPrefix='Rocky20240826_V2MT_g0';
% 
...
% 
% 
%  %timingfilename='/mnt/NPX/Rocky/20240826/Rocky20240826_V2_g0/Rocky20240826_V2_g0_t0.nidq.bin';
...
% 
% %%
% if isempty(dir('/home/huklab/Documents/NPX_pilot/Output/RockyV2MT_20240826/'))
%     mkdir('/home/huklab/Documents/NPX_pilot/Output/RockyV2MT_20240826/')
% end
% % save('/home/huklab/Documents/NPX_pilot/Output/RockyV2MT_20240826/Rocky20240826_V2_g0_t0.nidq_imecAp.mat','riseSent','riseRec','imecSent','imecSync','Event')
% % clear Tout
%  %load('/home/huklab/Documents/NPX_pilot/Output/RockyV2MT_20240826/Rocky20240826_V2_g0_t0.nidq_imecAp.mat')
% %%
% 
% riseRec0=riseRec;
% riseSent0=riseSent;
% 
% plot(diff(riseRec0),'.')
% plot(13585:13598,riseRec0(13585:13598),'.')
% 
% %riserec:nidaq received up until 13591,then after 13593 imec may have lost these though
% starts_sent=13579;
% starts_rec=13591;
% %risesent:imec has data up to 13579, then ok after 13580
% remaining_sent=length(riseSent0(13580:end));
% remaining_rec=length(riseRec0(13593:end));
% remaining=min(remaining_sent,remaining_rec);
% %%
% %555/60-> 9.25minutes not recorded!!
% 
%  %% Skips bad recording times, THIS ONLY SYNCs imec and nidaq, two seperate syncs are needed for PTB syncing
% %riseRec0=riseRec;
% riseRec=riseRec0([1:13579 end-remaining+1:end]);
% %riseSent0=riseSent;
% riseSent=riseSent0([1:13579 end-remaining+1:end]);
% 
...
% 
% %save(['/home/huklab/Documents/NPX_pilot/Output/RockyV2MT_20240826/' 'Rocky20240826_V2_t0.nidq' 'dp_ev_ts2_re.mat'],'Event')
% load(['/home/huklab/Documents/NPX_pilot/Output/RockyV2MT_20240826/' 'Rocky20240826_V2_t0.nidq' 'dp_ev_ts2.mat'],'Event')
% 
% %% LAST VALID TIMEpoints, function for time conversion needs to be piecewise
% last_samp1 =find(Event.samplenumber<riseRec(13579),1,'last');
% first_samp2=find(Event.samplenumber>riseRec(13580),1,'first');
% 
% last_ts1 =interp1(riseRec,riseSent,Event.samplenumber(last_samp1))/30000.076569037657;
% first_ts2=interp1(riseRec,riseSent,Event.samplenumber(first_samp2))/30000.076569037657;
% 
...
% 
% StimPath='/home/huklab/Documents/NPX_pilot/Output/RockyV2MT_20240826/Rocky20240826/';%/mnt/NPX/Rocky/20240826/Rocky20240826/';
...
% scatter(RecWords_ts,RecWords,'.')
% ylim([0 64]);
% %xlim([min(RecWords_ts) min(RecWords_ts)+ptbtime1-ptbtime0]); % ylim([0 64]);
% subplot(212)
% plot(ptbt_,ptbwords,'.')
% ylim([0 64]);
% 
...
% hold on
% pause(1)
% scatter(ptbst+myfun(0)-555,(ptbsw))
% hold off
% 
% %% check
% clf
...
% 
% %% Just first part
% RecWords_ts0=RecWords_ts;
% RecWords0=RecWords;
% 
% %%
% RecWords_ts1=RecWords_ts0(RecWords_ts0<last_ts1);
% RecWords1=RecWords0(RecWords_ts0<last_ts1);
% 
% RecWords_ts2=RecWords_ts0(RecWords_ts0>first_ts2);
% RecWords2=RecWords0(RecWords_ts0>first_ts2);
% 
% %% Correcting for drift, best to take matches
% RecWords_ts=RecWords_ts1;
% RecWords=RecWords1;
% figure(2);clf;
% wordstocheck=intersect(unique(ptbwords),unique(RecWords));
...
% end
% 
% tdiffmatches=cell2mat(tdiff);
% pmatches=cell2mat(ptb_match);
% ematches=cell2mat(rec_match);
% 
% pmatches=pmatches(tdiffmatches<0.1);
% ematches=ematches(tdiffmatches<0.1);
% 
% [fun1, inv1] =synchtime.align_clocks(ptbt(pmatches), RecWords_ts(ematches));
% plot(ptbt(pmatches), RecWords_ts(ematches), 'o'); hold on
% plot(xlim, fun1(xlim), 'k')
% xlabel('PTB clock')
% ylabel('Ephys clock')
% %%
% %fun1=fun;
...
%     if nnz(ptb_ind)>nnz(rec_ind) % more ptb, loop through rec
%         for jj=1: nnz(rec_ind)
%             [tdiff{ii}(jj),ptb_match{ii}(jj)]=min(abs(ptbt+myfun(0)-555-RecWords_ts(rec_ind(jj))));
%             rec_match{ii}(jj)=rec_ind(jj);
%         end
%     else % more rec, loop through ptb
%         for jj=1: nnz(ptb_ind)
%             [tdiff{ii}(jj),rec_match{ii}(jj)]=min(abs(ptbt(ptb_ind(jj))+myfun(0)-555-RecWords_ts));
%             ptb_match{ii}(jj)=ptb_ind(jj);
%         end
%     end
%     max(tdiff{ii})
...
% % ematches=ematches(tdiffmatches<0.1);
% 
% [fun2, inv2] = synchtime.align_clocks(ptbt(pmatches), RecWords_ts(ematches));
% plot(ptbt(pmatches), RecWords_ts(ematches), 'o'); hold on
% plot(xlim, fun2(xlim), 'k')
% xlabel('PTB clock')
% ylabel('Ephys clock')
% %%
% 
% %%
% fun_full = @(x) synchtime.combine_functions(fun1,fun2,inv1(last_ts1),inv2(first_ts2),x);
% RecWords_ts=RecWords_ts0;
% RecWords=RecWords0;
% %%
% clf
% x1=lowbound; x2=highbound;
...
% % Load facecall data
% % dataPath1 ='/mnt/NPX/Rocky/20240826/Rocky 2024-08-26/facecal';
...
% EyeExp=Exp;
% EyeExp.D=Exp.D(caltrials(1:end));
% %%
% C = calibGUI(EyeExp,'use_smo','true');
% C.cmat= [  1.1644    0.9533   -5.9989   -0.6757   0.5744];
% 
% %%
% clear C
...
% cmat=[ 3.2443    3.3932   -7.3412   18.3972  -81.1366];
% C.cmat=cmat
% %%
% clear C
...
Exp.FileTag = 'Rocky20240826_redo.mat';
%save(fullfile('/home/huklab/Documents/NPX_pilot/Output', Exp.FileTag), '-v7.3', '-struct', 'Exp')
Exp=load(fullfile('/media/huklab/DATA/NPData/NPX_pilot/Output/', Exp.FileTag));
%% Load spikes
%dataPath = '/home/huklab/Documents/NPX_pilot/Output/RockyV2MT_20240826/';
%dataPath = '/home/huklab/Documents/SpikeSorting/Output/Rocky20240826_V2MT_g0_imec0/kilosort4/';
dataPath='/media/huklab/Data/NPX/Ryansorting/Rocky/pipeline_results_Rocky20240826_V1V2_g0_imec1_locar_40_140/cur/cur_sorter_output/';
Exp.osp = load(fullfile(dataPath, 'spkilo.mat'));
Exp.osp.st = Exp.osp.st;%+0.06;
...
% % %% Second probe
% % dataPath = '/home/huklab/Documents/SpikeSorting/Output/Rocky/20240826-imec1';
...
% %% Retry with handful
% %% Load spikes
% dataPath = '/home/huklab/Documents/SpikeSorting/Output/2024-08-26/Retry';
% Exp.osp = load(fullfile(dataPath, 'spkilo.mat'));
...
% 
% Exp=load('/home/huklab/Documents/NPX_pilot/Rock20240826_V1V2_onlinedpi.mat');
% resync=load('/home/huklab/Documents/NPX_pilot/Rocky20240826_resync.mat');
% Exp.ptb2Ephys=resync.fun;
% Exp.FileTag='Rocky20240826_V1V2_resorted';
% %% Load spikes
% dataPath = '/home/huklab/Documents/SpikeSorting/Output/2024-08-26_FullyResorted/Rocky_2024-08-26-prb1/';
% Exp.osp = load(fullfile(dataPath, 'spkilo.mat'));
% Exp.osp.st = Exp.osp.st;
% Exp.osp.st(Exp.osp.st<0)=0;
% % move zero to the end
...
% 
% %% Second probe
% dataPath = '/home/huklab/Documents/SpikeSorting/Output/2024-08-26_FullyResorted/Rocky_2024-08-26-prb2/';
% Exp.osp2 = load(fullfile(dataPath, 'spkilo.mat'));
% Exp.osp2.st = Exp.osp2.st;
% Exp.osp2.st(Exp.osp2.st<0)=0;
% % move zero to the end
...
%      title(Exp.osp.cids(goodunits(cc)))
    ...
    title(sprintf('Unit: %d, depth: %4.f', Exp.osp.cids(cc), 4500-Exp.osp.clusterDepths(goodunits((cc)))))
...
for fig=20+(1:nfigs)
    ...
    fig_=fig-20;
    set(gcf,'Position',[0 0 1080 1080])
saveas(figure(fig),['/home/huklab/Documents/NPX_pilot/Output/RockyV2MT_20240826/Rocky20240826_V2MT_dotRFs_imec1_redo/Rocky20240826_V2MT_dotRFs_imec1_redo' num2str(nperfig*(fig_-1)+1) '_' num2str(nperfig*fig_) '.pdf'])
end
...
%        title(Exp.osp.cids(goodunits(ccid(cc))))
        ...
        %NOTE: depth is -ve (distance up from tip) so +ve is up
...
figure(31);clf
hold off
plot(Exp.vpx.smo(:,1),Exp.vpx.smo(:,2))
hold on
plot(Exp.vpx.dpi(:,1),Exp.vpx.dpi(:,2)/3.75 - 6.5)
xlim([1724720715.1264 1724720723.0273])
%%
figure(31)
hold off
plot(Exp.vpx.smo(:,1),Exp.vpx.smo(:,3))
hold on
plot(Exp.vpx.dpi(:,1),-Exp.vpx.dpi(:,3)/3.75 + 22)
xlim([1724720715.1264 1724720723.0273])
%%
Exp.vpx.onlinesmo=Exp.vpx.smo;
Exp.vpx.onlineraw=Exp.vpx.raw;
Exp.vpx.dvadpi(:,1)=Exp.vpx.dpi(:,1);
Exp.vpx.dvadpi(:,2)=Exp.vpx.dpi(:,2)/3.75 - 6.5;
Exp.vpx.dvadpi(:,3)=-Exp.vpx.dpi(:,3)/3.75 + 22;
%%
figure(31)
hold off
plot(Exp.vpxR.onlinesmo(:,1),Exp.vpxR.onlinesmo(:,3),'k')
hold on
% plot(Exp.vpx.dvadpi(:,1),Exp.vpx.dvadpi(:,3),'b')
% hold on
plot(Exp.vpx.smo(:,1),Exp.vpx.smo(:,3),'r')
xlim([1724720715.1264 1724720723.0273])
%%
% Exp.vpx.dvadpi(:,2)=2*Exp.vpx.dvadpi(:,2)+2;
% Exp.vpx.dvadpi(:,3)=1.8*Exp.vpx.dvadpi(:,3)+2.2;
%% Update
addpath('/home/huklab/Documents/yates-beyond-fixation/code')
fix_mitchelllab_exports2
%%
% Exp.vpx.smo(:,1)=Exp.vpx.dvadpi(:,1);
% Exp.vpx.smo(:,2)=Exp.vpx.dvadpi(:,2);
% Exp.vpx.smo(:,3)=Exp.vpx.dvadpi(:,3);
%% Remove some bad timepoints (when eyetrack and ephys don't match; recording was paused)
badtracking=isnan(Exp.vpx2ephys(Exp.vpx.smo(:,1)));
Exp.vpx.smo=Exp.vpx.smo(~badtracking,:);
Exp.vpx.Labels(badtracking)=[];
%%

% convert to D struct format
cd('/home/huklab/Documents/NPX_pilot/V1FreeViewingCode/MatlabCode')
...
gratSF = D.GratingFrequency(gratIx);
gratTF = D.GratingSpeeds(gratIx);
...
for iG = 1:nGratings
    plot([gratOnsets(iG) gratOffsets(iG)], gratSF(iG)*[1 1], 'r', 'Linewidth', 2); hold on
end
ylabel('SF')
...
for sf=1:numel(unique(gratSF))
    spikeCond(sf,:)=mean(spikeRate(gratSF==(2.^(sf-2)),:));
end
%%
for cc=1:NC
    plot(spikeCond(:,cc))
...
treadSpeed = abs(D.treadSpeed(treadIx));
runThresh=1;
...
%%
% StimPath='/home/huklab/Documents/NPX_pilot/Output/RockyV2MT_20240826/Rocky20240826/';%/mnt/NPX/Rocky/20240826/Rocky20240826/';
% Exp_check = io.basic_marmoview_import(StimPath);
% %%
% for jj=1066:1096
%     Exp.D{jj}=Exp_check.D{jj};
% end

%% ~~~~~~~EXTRACTING DATA FOR FIELD COLAB
%Exp.D{validTrials(ii)}.eyeData(8:12487,1);
% %%
% validTrials = io.getValidTrials(Exp,'BackVideo');
% % Extract trial-specific values
% frameTimes = cellfun(@(x) x.eyeData(8:12487,1), Exp.D(validTrials(:)), 'uni', 0);
% 
...
% frameTimes = Exp.ptb2Ephys(cell2mat(frameTimes));
% keepframes=~isnan(frameTimes);
% frameTimes = frameTimes(keepframes);
% 
% % bin spikes
% Robs = binNeuronSpikeTimesFast(Exp.osp, frameTimes, 1/60);
% 
%     cids = Exp.osp.cids;
% 
...
% 
% %% Package it up
% NTr=length(validTrials);
% NFr=12480;
% NC=size(Robs,2);
% fillrobs=nan(length(keepframes),NC);
% fillrobs(keepframes,:)=Robs;
% filleyes=nan(length(keepframes),2);
% filleyes(keepframes,:)=eyeAtFrame;
% 
% eyepos_out=permute(reshape(filleyes,NFr,NTr,2),[1,2,3]);
% rate_out=permute(reshape(fillrobs,NFr,NTr,NC),[1,2,3]);
%%
...
for ii=1:length(validTrials)
t(:,ii)=Exp.D{validTrials(ii)}.eyeData(8:12487,1);
ex(:,ii)=Exp.D{validTrials(ii)}.eyeData(8:12487,2);
ey(:,ii)=Exp.D{validTrials(ii)}.eyeData(8:12487,3);
end

c=Exp.D{validTrials(ii)}.C.c;
dx=Exp.D{validTrials(ii)}.C.dx;
dy=Exp.D{validTrials(ii)}.C.dy;

x = (ex-c(1)) / (dx*Exp.S.pixPerDeg);
y = (ey-c(2)) / (dy*Exp.S.pixPerDeg);
...
dxdt=[0; diff(x(:,ii))];
dydt=[0; diff(y(:,ii))];
bad=find(dxdt>10|dydt>10|abs(x(:,ii))>15|abs(y(:,ii))>15);
bad(bad<2)=2;bad(bad>12478)=12478;
x(bad+[-1:1],ii)=nan;
y(bad+[-1:1],ii)=nan;
end

[Y, binfun] = binNeuronSpikeTimesFast(sp, eventTimes, binSize, keep_sparse)

% save('Eyetrack_Rocky20240826.mat','x','y')
%%

figDir='/home/huklab/Documents/NPX_pilot/Output/RockyV2V1_20240826';
...
figure(131); clf; set(gcf, 'Color', 'w')
...
stat0 = tuning_empirical(D, 'binsize', 10e-3, ...
    'runningthresh', 1, ...
    'nboot', 500, ...
    'seed', 1234);  
%%
stat = tuning_empirical2(D, 'binsize', 10e-3, ...
    'runningthresh', 1, ...
    'nboot', 500, ...
    'seed', 1234);  

%% Full out full model for each sampled point, could even upsample 
pmap=[[ones(1,128) linspace(1,0,128)]; [linspace(0,1,128) linspace(1,0,128)];[linspace(0,1,128) ones(1,128)]]';
sfax = unique(D.GratingFrequency);
tfax = unique(D.GratingSpeeds);
NC=length(stat.TCfitS);
Residual_mat=nan(length(sfax),length(tfax),NC);
gain_mat=nan(length(sfax),length(tfax),NC);
n_points_res=nan(length(sfax),length(tfax),NC);
Residual_av=zeros(length(sfax),length(tfax),1);
% 
sx = ceil(sqrt(NC));
sy = round(sqrt(NC));
...
for cc = 60:77 % all 1:NC, %subset 60:77
    fprintf('%d/%d\n', cc, NC)
    set(gcf, 'currentaxes', ax(cc))
    if (stat.residualS_after_fitS(cc).rsquared)<(-1.3)
        continue
    end
    for sf=1:length(sfax)
        sfix=stat.residualR_after_fitS(cc).sf==sfax(sf);
        for tf=1:length(tfax)
            tfix=stat.residualR_after_fitS(cc).tf==tfax(tf);
            Residual_mat(sf,tf,cc)=nansum(stat.residualR_after_fitS(cc).res(sfix&tfix));
            gain_mat(sf,tf,cc)=nansum(stat.residualR_after_fitS(cc).gain(sfix&tfix));
            n_points_res(sf,tf,cc)=nansum(sfix&tfix); 
        end
    end
    Residual_av=Residual_av+Residual_mat(:,:,cc)./n_points_res(:,:,cc);
    gain_av=gain_av+gain_mat(:,:,cc)./n_points_res(:,:,cc);
...
figure(100);
imagesc(mean(gain_mat./n_points_res,3,'omitnan')'); colorbar; colormap(pmap)
xlabel('SF');  xticklabels({'0.5', '1', '2', '4', '8'});
ylabel('TF');  yticklabels({'','0.5', '', '1','', '2','', '4','', '8'});
clim([-1.2 1.2])
title('Incr in rate: mean(residual/baseline)'); 
axis equal tight xy

%% Attempt to fully predict rates
do_regression_analysis_2

%% Full out full model for each sampled point, could even upsample 
thax = unique(D.GratingDirections);
sfax = unique(D.GratingFrequency);
tfax = unique(D.GratingSpeeds);
thax=0:10:360;sfax=2.^(-1:0.1:3);tfax=2.^(-1:0.1:3);
NC=length(stat.TCfitS);
Stationary_tuning_model=nan(length(thax),length(sfax),length(tfax),NC);


for cc = 1:NC
    fprintf('%d/%d\n', cc, NC)
    for th=1:length(thax)
        for sf=1:length(sfax)
            for tf=1:length(tfax)
                Stationary_tuning_model(th,sf,tf,cc)=stat.TCfitS(cc).tuningFun(thax(th),sfax(sf),tfax(tf));
            end
        end
    end
end
...
for cc=1:length(stat.TCfitS)
    fprintf('%d/%d\n', cc, NC)
    set(gcf, 'currentaxes', ax(cc))
% figure(900);clf;
% subplot(221)
% imagesc(thax,tfax,squeeze(mean(Stationary_tuning_model(:,:,:,cc),2))')
% subplot(222)
% imagesc(thax,sfax,squeeze(mean(Stationary_tuning_model(:,:,:,cc),3))')
% title(num2str(cc))
% subplot(223)
contourf(sfax,tfax,squeeze(mean(Stationary_tuning_model(:,:,:,cc),1)))
title(sprintf('Unit: %d', Exp.osp.cids(cc)))
axis xy equal tight
title(num2str(cc))
pause(0.01)
...
figure(111); clf; set(gcf, 'Color', 'w')
ax = plot.tight_subplot(sy, sx, 0.01, 0.01);
...
inds = io.getValidTrials(Exp, 'BackVideo');
...
%%
figure(99)
msr1=mean(spikeRate2(:,:,10000:35000),3);
msr2=mean(spikeRate2(:,:,40000:50000),3);
msr=mean(spikeRate2,3);
figure(1);clf
%%
tv=linspace(-1,+1,100);
...
%
subplot(212)
plot(tv(2:end),mean(msr1'))

%%
figure(100);clf
imagesc((msr./median(msr))')
