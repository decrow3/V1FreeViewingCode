%% Additional eye analysis for NPX datasets
...
subjects = {'npx_V1_fov_eg','npx_V1_calc_eg'};%, 'allen'};
...
subjects = {'npx_V1_fov_eg', 'npx_V1_calc_eg'};
nsubjs = numel(subjects);
for isubj = 1:nsubjs
    subject = subjects{isubj};
    cd('/home/huklab/Documents/NPX_pilot/V1Locomotion/Code/')
...
%%
figdir = '/home/huklab/Documents/NPX_pilot/V1FreeViewingCode/Figures/Eyestats';
nboot = 100;
...
%        ci = bootci(nboot, @geomean, SacMgR(:,2)./SacMgS(:,2));
        ...
        ExpectedFRDelta=SacHzDelta.*Stat.(subject).regRnSac_B(good,1); %Per cell expected change in firing rate
        ExpectedFRSacHzR=SacHzR.*Stat.(subject).regRnSac_B(good,1)+Stat.(subject).regRnSac_B(good,2);
        ExpectedFRSacHzS=SacHzS.*Stat.(subject).regRnSac_B(good,1)+Stat.(subject).regRnSac_B(good,2);
        ExpectedFRDelta2=ExpectedFRSacHzR-ExpectedFRSacHzS;
        fprintf("Expected delta (running-stationary) from reg. on nSaccades: %.4f Hz \n" ,nanmean(ExpectedFRDelta2(good,2)))
        
        speedR=Stat.(subject).speedR(good,:);
        speedS=Stat.(subject).speedS(good,:);
        speedDelta=(speedR-speedS); %Per cell diff in saccades
        ExpectedFRspeedDelta=speedDelta.*Stat.(subject).regRnSac_B(good,1); %Per cell expected change in firing rate
        ExpectedFRspeedR=speedR.*Stat.(subject).regRnSpd_B(good,1)+Stat.(subject).regRnSpd_B(good,2);
        ExpectedFRspeedS=speedS.*Stat.(subject).regRnSpd_B(good,1)+Stat.(subject).regRnSpd_B(good,2);
        meangainsac=mean(ExpectedFRspeedR(good,2)./ExpectedFRspeedS(good,2),'omitnan');
        fprintf("Expected delta (running-stationary) from reg. on run speed: %.4f Hz \n" ,nanmean(ExpectedFRspeedDelta(good,2)))
        
        %
        FRDeltaStim=frStimR(good,2)-frStimS(good,2);
        meangain=mean(frStimR(good,2)./frStimS(good,2),'omitnan');
        ...
        ExpectedFRSacHzR=SacHzR.*Stat.(subject).regRnSac_B(good,1)+Stat.(subject).regRnSac_B(good,2);
        ExpectedFRSacHzS=SacHzS.*Stat.(subject).regRnSac_B(good,1)+Stat.(subject).regRnSac_B(good,2);
        meangainsac=mean(ExpectedFRSacHzR(good,2)./ExpectedFRSacHzS(good,2),'omitnan');
        fprintf("Expected gain (running/stationary) from reg. on nSaccades: %.4f  \n" ,meangainsac)
        
        
        speedDelta=(speedR-speedS); %Per cell diff in saccades
        ExpectedFRDelta=speedDelta.*Stat.(subject).regRnSac_B(good,1); %Per cell expected change in firing rate
        ExpectedFRspeedR=speedR.*Stat.(subject).regRnSpd_B(good,1)+Stat.(subject).regRnSpd_B(good,2);
        ExpectedFRspeedS=speedS.*Stat.(subject).regRnSpd_B(good,1)+Stat.(subject).regRnSpd_B(good,2);
...
% Foveal
subjects0={'npx_V1_fov_eg', 'npx_V1_calc_eg'};
%%
subjects={subjects0{1}};nsubjs=length(subjects);
asidecombine_good
%%
% Periph
subjects={subjects0{2}};nsubjs=length(subjects);
asidecombine_good

%%
% Recombined
subjects=subjects0;nsubjs=length(subjects);
%asidecombine_good
