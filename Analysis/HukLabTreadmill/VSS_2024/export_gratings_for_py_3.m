...
%%
%Data factory is quite eloborate
    %Exp=load('/home/huklab/Documents/NPX_pilot/brie_b20220625_prb1.mat');
% Exp=load('/media/huklab/Data/NPX/HuklabTreadmill/gru/g20220412.mat') %bad version

% Exp=load('/home/huklab/Documents/NPX_pilot/V1FreeViewingCode/b20220809re2.mat');
%Exp=load('/home/huklab/Documents/NPX_pilot/V1FreeViewingCode/brie_b20220625_prb1.mat');
% Exp=load('/media/huklab/Data/NPX/HuklabTreadmill/VSS/brie_20220701_probe2_fov.mat');
%Exp=load('/media/huklab/Data/NPX/HuklabTreadmill/brie/b20220701_probe2.mat'); %full probe
% Exp=load('/media/huklab/Data/NPX/HuklabTreadmill/VSS/brie_b20220625_prb2.mat');
%Exp=load('/media/huklab/Data/NPX/HuklabTreadmill/brie/b20220625_probe1.mat');
% Exp=load('/media/huklab/Data/NPX/HuklabTreadmill/VSS/brie_20220625_probe1_fov')
%Exp=load('/media/huklab/Data/NPX/HuklabTreadmill/VSS/brie_20220625_probe1_calc1.mat')
%Exp=load('/media/huklab/Data/NPX/HuklabTreadmill/VSS/brie_20220625_probe1_calc2.mat');
%Exp=load('/media/huklab/Data/NPX/HuklabTreadmill/VSS/brie_20220625_probe1_fov2')

% sessList={'/media/huklab/Data/NPX/HuklabTreadmill/VSS/brie_20220625_probe1_fov', ...
%    '/media/huklab/Data/NPX/HuklabTreadmill/VSS/brie_20220625_probe1_calc1'};%, ...
% %     '/media/huklab/Data/NPX/HuklabTreadmill/VSS/brie_20220625_probe2_calc2', ...
%     '/media/huklab/Data/NPX/HuklabTreadmill/VSS/brie_20220625_probe2_fov2'};

%sessList={'/media/huklab/Data/NPX/HuklabTreadmill/VSS/brie_20220809_MTshallow.mat'};
%sessList={'/media/huklab/Data/NPX/HuklabTreadmill/gru/g20220412.mat'};
%sessList={'/home/huklab/Documents/NPX_pilot/Rock20231209_V1V2_probe1.mat'};
%sessList={'/media/huklab/Data/NPX/HuklabTreadmill/VSS/rocky_20231209_probe1_fov.mat'};
sessList={'/media/huklab/Data/NPX/HuklabTreadmill/VSS/g20220412re3.mat'};
%% Step 1.1: Try importing a session


for id = 1:length(sessList)
    Exp=load(sessList{id});
   ...
   % try
        ...
        if isfield(Exp, 'D') && isfield(Exp.D{1}, 'inputs')
        for trial=1:length(Exp.D)
            if isa(Exp.D{trial}.inputs{2},'marmoview.treadmill_arduino')
                Exp.D{trial}.treadmill=Exp.D{trial}.inputs{2};
            end
        end
        end
       ...
       D = io.get_drifting_grating_output(Exp);
        ...
        if isfield(Exp, 'D')
            ...
            if ~isempty(dotTrials)
                ...
                RFs = spat_rf_helper(Exp, 'ROI', BIGROI, ...
                    'win', [-5 12],...
                    'binSize', binSize, 'plot', true, 'debug', false, 'spikesmooth', 0);
                
                Exp1=Exp; %merge all spikes into a single unit
                Exp1.osp.clu = ones(size(Exp.osp.clu));
                Exp1.osp.cids = 1;
                Exp1.spikeIds = Exp1.osp.clu;

                popRF = spat_rf_helper(Exp1, 'ROI', BIGROI, ...
                    'win', [-5 12],...
                    'binSize', binSize, 'plot', true, 'debug', false, 'spikesmooth', 0);
                ...
                % PLOT RFS
                hasrf = find(RFs.area > binSize & RFs.maxV > 2);
        ...
        % save file
        splited=split(Exp.FileTag,'/');
        if length(splited)>1
            Exp.FileTag=splited{end};
        end
        fname = strrep(Exp.FileTag, '.mat', '_grat.mat');
        fdir = fullfile(getpref('FREEVIEWING', 'HUKLAB_DATASHARE'), 'VSS');
...
figure;
hold on
for ii=1:length(RFs.cids)
if ~isempty(RFs.contours{ii,1})
plot3(RFs.contours{ii,1}(:,1),RFs.contours{ii,1}(:,2),repmat(Exp.osp.clusterDepths(RFs.cids(ii)),length(RFs.contours{ii,1}(:,2)),1))
end
end
save(fullfile(fdir, [fname '_RFs']), '-v7.3', '-struct', 'RFs')

%% 
% units=D.units
% crf=[units(~isnan([units.maxV])).center];
% plot(hypot(crf(1:2:end),crf(2:2:end)))
% cids1=unique(D.spikeIds)
% remove=cids1(hypot(crf(1:2:end),crf(2:2:end))>5)
% fovbool=~ismember(D.spikeIds,remove);
% D.spikeTimes=D.spikeTimes(fovbool);
% D.spikeIds=D.spikeIds(fovbool);
% remove=cids(hypot(crf(1:2:end),crf(2:2:end))>5)
% units(remove)=[];
% %%
% 
% 
% crf=[units(~isnan([units.maxV])).center];
% plot(crf)
% if nnz(hypot(crf(1:2:end),crf(2:2:end))>5)==0
% D.units=units;
% end
% %%
% nnz(D.GratingDirections==135)
% D.GratingDirections(D.GratingDirections==135)=nan;
%%
% 
% 
