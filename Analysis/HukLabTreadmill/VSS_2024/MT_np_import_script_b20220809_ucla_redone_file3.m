...
%%
load('/home/huklab/Documents/SpikeSorting/Output/2022-08-09-file3re_12_8/rez.mat')

tt=max(rez.st3(:,1))
tbatch=tt/rez.ops.Nbatch
dshifttimes=(1:tbatch:tt);
dshift=rez.dshift;
save('/home/huklab/Documents/SpikeSorting/Output/2022-08-09-file3re_12_8/shifts.mat','dshift','dshifttimes')
subplot(8,1,[1:7])
...
dataPath = fullfile(HUKDATA, 'BrieMTsessionf3','20220809');
figDir = fullfile(HUKDATA, 'BrieMTsessionf3','20220809','out');%'~

%%
% File combiner ran and sorted, files saved to:
dataPath= '/home/huklab/Documents/SpikeSorting/Output/2022-08-09-file3re_12_8/';
...
%%
save('b20220809_quicksaveln500_re_f3_.mat','-v7.3')
%%
% %%
...
%%
save('b20220809_quicksaveln825_file3re_12_8_re.mat','-v7.3')
...
%    
    ...
    iix = ~(isnan(opts.lags));
    frall = sum(robs(:,iix),2) / (max(opts.lags(iix)) - min(opts.lags(iix)));

    frAllR(cc,:) = prctile(mean(frall(runTrials(randi(nrun, [n nboot])))), [2.5 50 97.5]);
    frAllS(cc,:) = prctile(mean(frall(statTrials(randi(nstat, [n nboot])))), [2.5 50 97.5]);    
    end
end
...
SacHzR = prctile(mean(nSac(runTrials(randi(nrun, [n nboot])))), [2.5 50 97.5])./0.3;
SacHzS = prctile(mean(nSac(statTrials(randi(nstat, [n nboot])))), [2.5 50 97.5])./0.3;
SacHzAll = prctile(mean(nSac((randi(nstat, [n nboot])))), [2.5 50 97.5])./0.3;
...
xlabel('Stationary')
ylabel('Running')
title('MT: Baseline Firing Rate')

set(gca, 'Xscale', 'log', 'Yscale', 'log')
plot(xlim, xlim, 'k')
xlim1=[0.1 100];
xlim(xlim1);ylim(xlim1);
...
title('MT: Stim-driven firing rate')
...
m = geomean(frAllR(:,2)./frAllS(:,2));
 ci = bootci(nboot, @geomean, frAllR(:,2)./frAllS(:,2));
%ci = nan(1,2);
fprintf("geometric mean overall firing rate ratio (Running:Stationary) is %02.3f [%02.3f, %02.3f] (n=%d)\n", m, ci(1), ci(2), NC)


EffectBase=(frBaseR(:,2)-frBaseS(:,2))./(frBaseS(:,2));
avEffectBase=100*mean(EffectBase);
SEMEffectBase = std(EffectBase)/sqrt(length(EffectBase));

EffectStim= (frStimR(:,2)-frStimS(:,2))./(frStimS(:,2));   
avEffectStim=100*mean(EffectStim);
SEMEffectStim = 100*std(EffectStim)/sqrt(length(EffectStim));

AbsEffectBase=abs(frBaseR(:,2)-frBaseS(:,2))./(frBaseS(:,2));
avAbsEffectBase=100*mean(AbsEffectBase);
SEMAbsEffectBase = 100*std(AbsEffectBase)/sqrt(length(AbsEffectBase));

AbsEffectStim=abs(frStimR(:,2)-frStimS(:,2))./(frStimS(:,2));
