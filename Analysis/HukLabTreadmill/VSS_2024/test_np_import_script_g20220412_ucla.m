...
figDir = '~/Google Drive/HuklabTreadmill/imported_sessions_qa/gru_20220412/';
%%
...
%% Reorder D struct
Dtrialstarts=cellfun(@(x) x.STARTCLOCKTIME, Exp.D);
[~, TrialOrder]=sort(Dtrialstarts);

for ii=1:length(TrialOrder)
D2{ii,1}=Exp.D{TrialOrder(ii)};
end
Exp.D=D2;
%% Import eye position signals
dataPath = '/media/huklab/Data/NPX/HuklabTreadmill/gru/g20220412'; %~/Google Drive/HuklabTreadmill/gru/g20210521/';
fid = 1;
[Exp0, fig] = io.import_eye_position(Exp, dataPath, 'fid', fid, 'zero_mean', true, 'normalize', true);

%% QUICKSAVE!!
Exp0 = Exp; % backup here
%save('quicksaveln198.mat','-v7.3')


%%
%% Load facecall data without sync
dataPath1 = '/media/huklab/Data/NPX/HuklabTreadmill/gru/g20220412/FaceCal - not recorded';
...
%o2=load('C:\Users\Nauhaus Lab\Documents\MATLAB\NPX_pilot\MarmoV5\SupportData\MarmoViewLastCalib.mat');
o2.dx= 2.1896e-04;
o2.dy= 2.3426e-04;
o2.c=[0.0750    1.0683];
...
% cmat0=[43.2047   26.7550   -2.2550    1.2397    0.6979];
%%
%       scalex   scaley               shifty shifty 
% %WORKING
% C.cmat=[-0.250    -0.1750    0.2000      .45   -0.15];
% %FLIP Y
...
%  %
%  figure(1);hold off
%  plot(eyePos(1:20:end,1),eyePos(1:20:end,2),'r.')
% xx=meshgrid(-20:5:20,-20:5:20);
% yy=meshgrid(20:-5:-20,20:-5:-20)';
% hold on
% scatter(xx(:),yy(:),'k','linewidth',5);
% 
...
%% SURELY THERE NEEDS TO BE A SUBSTANTIAL OFFSET FOR THE FILETIME??
...
%%
dataPath='/home/huklab/Documents/SpikeSorting/Output/Gru/20220412/1/';
figDir=dataPath;
Exp.spikeTimes = Exp.osp.st;
