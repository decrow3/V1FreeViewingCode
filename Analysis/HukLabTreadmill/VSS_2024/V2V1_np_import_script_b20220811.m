...
%V2V1 - single probe (first), looking for RFs
'/home/declan/data/BrieV1V2-0811/20220811/ephys/2022-08-11_13-29-17/Neuropix-PXI-101.0/kilosort/'
...
dataPath = fullfile(HUKDATA, 'BrieV1V2-0811','20220811');
figDir = fullfile(HUKDATA, 'BrieV1V2-0811','20220811','out');%'~/Google Drive/HuklabTreadmill/imported_sessions_qa/gru_20220516/';
%%
dataPath = '/home/declan/data/BrieV1V2-0811/20220811/ephys/2022-08-11_13-29-17/Rhythm_FPGA-102.0/TTL_1/';
...
dataPath= '/home/declan/data/BrieV1V2-0811/20220811/stim/';
...
%% Just sync relevent file

toffset=1.6602448*10^9;
...
%save('b202200811_quicksaveln198.mat','-v7.3')


%%
%% Load facecall data without sync
dataPath1 = '/home/declan/data/BrieV1V2-0811/20220811/stim/Facecal/';
...
C.cmat=[0.0033   -0.0065    0.0036    0.0795   -0.0506];
C.cmat=[-0.0033   0.0065    0.0036    0.0695   -0.035];
%fig = io.checkCalibration(C.Exp);
% hold on
figure(1);hold off
eyePos =get_eyepos(C);
plot(eyePos(1:50:end,1),eyePos(1:50:end,2),'k.')
hold on
plot(x1,y1,'r.')
...
%hold off
 ...
 %% Refine by hand
% C.cmat=[-0.0033   0.0065    0.0036    0.0695   -0.035];
C.cmat=[-0.0033   -0.0065    0.0036    0.0805   -0.0516];
eyePos =get_eyepos(C);
 figure(1);hold off
...
plot(Exp.vpx.smo(1:500:end,2),Exp.vpx.smo(1:500:end,3),'.')
xx=meshgrid(-20:5:20,-20:5:20);
yy=meshgrid(20:-5:-20,20:-5:-20)';
hold on
scatter(xx(:),yy(:),'k');
...
axis equal;xlim([-10 10]);ylim([-10 10]);
%%
save('b20220811_quicksaveln452.mat','-v7.3')
%%
% %%
...
%% load spikes
dataPath = '/home/declan/data/BrieV1V2-0811/20220811/ephys/2022-08-11_13-29-17/Neuropix-PXI-101.0/kilosort/';
...
timingloc='/home/declan/data/BrieV1V2-0811/20220811/ephys/2022-08-11_13-29-17/Neuropix-PXI-101.0/';
...
%%
save('b20220811_quicksaveln825.mat','-v7.3')
...
%plot.fixfigure(gcf, 12, [8 11])
%%
figDir='/home/declan/data/BrieV2V1session/20220811/out/';
...
Exp.FileTag = 'brie/b20220811.mat'
