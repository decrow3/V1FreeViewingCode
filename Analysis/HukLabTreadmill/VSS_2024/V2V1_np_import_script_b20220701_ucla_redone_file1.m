...
%%
load('/home/huklab/Documents/SpikeSorting/Output/2022-07-01/rez.mat')
tt=max(rez.st3(:,1))
tbatch=tt/rez.ops.Nbatch
dshifttimes=(1:tbatch:tt);
dshift=rez.dshift;
%save('/home/huklab/Documents/SpikeSorting/Output/2022-07-01-Combined-full/shifts.mat','dshift','dshifttimes')
subplot(8,1,[1:7])
...
dataPath = fullfile(HUKDATA, 'BrieV2V1session','20220701');
figDir = fullfile(HUKDATA, 'BrieV2V1session','20220701','out');%'~
figDir=dataPath;
%%
% File combiner ran and sorted, files saved to:
% dataPath='/home/huklab/Documents/SpikeSorting/Output/2022-07-01-file1probe1/';
 dataPath= '/home/huklab/Documents/SpikeSorting/Output/2022-07-01-file1probe1/';
...
%%
save('b20220701_f1_quicksaveln500_re.mat','-v7.3')
%%
% %%
...
%% load spikes
% dataPath = '/home/declan/data/BrieV2V1session/20220701/ephys/2022-07-01_17-31-35/Neuropix-PXI-101.0/Kilosort/';
%dataPath = '/home/declan/data/BrieV2V1session/20220701/ephys/2022-07-01_12-44-58/experiment1/continuous/Neuropix-PXI-101.0/kilosort/';
...
%%
% timingloc='/home/declan/data/BrieV2V1session/20220701/ephys/2022-07-01_12-44-58/experiment1/continuous/Neuropix-PXI-101.0/';
...
%stimes=stimes(stimes>8555&stimes<10555);
...
subset = [250,251,319,323,368,564,602,613,740,772,798,813,818, ...
    847,852,1117,1126,1172,1182,1183,1201, 1387, 1388, 1389,1391,1392,1394,1403,1404];
nsub = numel(subset);
...
% timingloc='/home/declan/data/BrieV2V1session/20220701/ephys/2022-07-01_17-31-35/Neuropix-PXI-101.0/'
...
%%
save('b20220701_quicksaveln825_re.mat','-v7.3')
...
%plot.fixfigure(gcf, 12, [8 11])
%%
% figDir='/home/declan/data/BrieV2V1session/20220701/out/';
figDir=dataPath;
...
V1ind=(7000-Exp.osp.clusterDepths)>4000;
%% plot some outcomes

incBaseIx = find((frBaseR(:,2) > frBaseS(:,3)) & V1ind');
decBaseIx = find((frBaseR(:,2) < frBaseS(:,1)) & V1ind');

incStimIx = find((frStimR(:,2) > frStimS(:,3)) & V1ind');
decStimIx = find((frStimR(:,2) < frStimS(:,1)) & V1ind');
...
good = ~(frBaseR(V1ind,2)==0 | frBaseS(V1ind,2)==0);
...
m = geomean(frStimR(V1ind,2)./frStimS(V1ind,2));
% ci = bootci(nboot, @geomean, frStimR(:,2)./frStimS(:,2));
ci = nan(1,2);
fprintf("geometric mean stim-driven firing rate ratio (Running:Stationary) is %02.3f [%02.3f, %02.3f] (n=%d)\n", m, ci(1), ci(2), NC)
...
good = ~(frBaseR(V2ind,2)==0 | frBaseS(V2ind,2)==0);
...
m = geomean(frStimR(V2ind,2)./frStimS(V2ind,2));
% ci = bootci(nboot, @geomean, frStimR(:,2)./frStimS(:,2));
ci = nan(1,2);
fprintf("geometric mean stim-driven firing rate ratio (Running:Stationary) is %02.3f [%02.3f, %02.3f] (n=%d)\n", m, ci(1), ci(2), NC)
...
Exp.FileTag = 'brie_b20220701_f1.mat'
