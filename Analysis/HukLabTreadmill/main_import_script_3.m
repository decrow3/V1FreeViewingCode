...
%% Intervening for new runs
eyePos = [Exp.vpx.smo(:,3) Exp.vpx.smo(:,2)];
eyePos = -Exp.vpx.smo(:,2:3);
...
zlabel('depth')
%
ylabel('degrees'); ylim([-10 10]);
xlabel('degrees'); xlim([-10 10]);


%%
%Added later, dpr
factors = [1 2 3];
xax=stat2.xax; yax=stat2.yax;
...
clear stableIx sacmagspd sacmagtri sacmag corr* Sac* Pup* EyeVar*


for cc = 1:NC
    unitId = unitList(cc);
    [stim, robs, behav, opts] = bin_ssunit(D, unitId, 'win', [-.2 .2]);
    ...
    eyeFlag = behav{4}(goodIx,:); %Flag for eye state (saccade/fix/lost etc)
    ...
    [corrSacFRRho(cc), corrSacFRPval(cc)] = corr(nSac(~isnan(spd)), frbase(~isnan(spd)), 'type', 'Spearman');
    [corrSizeFRRho(cc), corrSizeFRPval(cc)] = corr(eyeSize(~isnan(spd)), frbase(~isnan(spd)), 'type', 'Spearman');
    [corrVarXFRRho(cc), corrVarXFRPval(cc)] = corr(varX(~isnan(spd)), frbase(~isnan(spd)), 'type', 'Spearman');
    [corrVarYFRRho(cc), corrVarYFRPval(cc)] = corr(varY(~isnan(spd)), frbase(~isnan(spd)), 'type', 'Spearman');
    [corrVarXYFRRho(cc), corrVarXYFRPval(cc)] = corr(varXY(~isnan(spd)), frbase(~isnan(spd)), 'type', 'Spearman');

    %Behaviour correlates of speed
    [corrSacRho(cc), corrSacPval(cc)] = corr(spd(~isnan(spd)), nSac(~isnan(spd)), 'type', 'Spearman');
    [corrSizeRho(cc), corrSizePval(cc)] = corr(spd(~isnan(spd)), eyeSize(~isnan(spd)), 'type', 'Spearman');
    [corrVarXRho(cc), corrVarXPval(cc)] = corr(spd(~isnan(spd)), varX(~isnan(spd)), 'type', 'Spearman');
    [corrVarYRho(cc), corrVarYPval(cc)] = corr(spd(~isnan(spd)), varY(~isnan(spd)), 'type', 'Spearman');
    [corrVarXYRho(cc), corrVarXYPval(cc)] = corr(spd(~isnan(spd)), varXY(~isnan(spd)), 'type', 'Spearman');
    ...
    SacHzR(cc,:) = prctile(mean(nSac(runTrials(randi(nrun, [n nboot])))), [2.5 50 97.5])./timebintotal;
    SacHzS(cc,:) = prctile(mean(nSac(statTrials(randi(nstat, [n nboot])))), [2.5 50 97.5])./timebintotal;
    SacHzAll(cc,:) = prctile(mean(nSac((randi(nstat, [n nboot])))), [2.5 50 97.5])./timebintotal;


    SacMgR(cc,:) = prctile(nanmean(MSac(runTrials(randi(nrun, [n nboot])))), [2.5 50 97.5])./timebintotal;
    SacMgS(cc,:) = prctile(nanmean(MSac(statTrials(randi(nstat, [n nboot])))), [2.5 50 97.5])./timebintotal;
    SacMgAll(cc,:) = prctile(nanmean(MSac((randi(nstat, [n nboot])))), [2.5 50 97.5])./timebintotal;

    PupSzR(cc,:) = prctile(mean(eyeSize(runTrials(randi(nrun, [n nboot])))), [2.5 50 97.5])./timebintotal;
    PupSzS(cc,:) = prctile(mean(eyeSize(statTrials(randi(nstat, [n nboot])))), [2.5 50 97.5])./timebintotal;
    PupSzAll(cc,:) = prctile(mean(eyeSize((randi(nstat, [n nboot])))), [2.5 50 97.5])./timebintotal;

    EyeVarR(cc,:) = prctile(mean(varXY(runTrials(randi(nrun, [n nboot])))), [2.5 50 97.5])./timebintotal;
    EyeVarS(cc,:) = prctile(mean(varXY(statTrials(randi(nstat, [n nboot])))), [2.5 50 97.5])./timebintotal;
    EyeVarAll(cc,:) = prctile(mean(varXY((randi(nstat, [n nboot])))), [2.5 50 97.5])./timebintotal;
