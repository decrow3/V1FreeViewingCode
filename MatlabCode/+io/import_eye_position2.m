...
%%
            ...
            %% Zero out filewise (saw some between-file drift)%% Zero out filewise (saw some between-file drift)
            vpx=Exp.vpx;

            if ip.Results.zero_mean
                
                FileBreak=find(diff(Exp.vpx.raw(~isnan(Exp.vpx.raw(:,1)),1))>10);
                FileBreak=[0 FileBreak' size(Exp.vpx.raw,1)];
                for jj=1:numel(FileBreak)-1
                    st=FileBreak(jj)+1;
                    ed=FileBreak(jj+1);
                    lost_track = vpx.raw(st:ed,2)==max(vpx.raw(st:ed,2)) | vpx.raw(st:ed,3)==max(vpx.raw(st:ed,3));

                    dx=[(diff(vpx.raw(st:ed,2))); 0];
                    dy=[(diff(vpx.raw(st:ed,3))); 0];
        
                    % Choosing 80 as a threshold, I don't know how well this
                    % will be conserved across experiments, DPR 5-22-22
                    thr=80;
                    assert(sum(abs(dx)>thr)/numel(dx)<.1,'import_eye_position: many large jumps are being removed, check the threshold')
                    jump_track=(abs(dx)>thr)|(abs(dy)>thr);
                    lost_track=lost_track|jump_track;

                    lost_track_full=ones(size(Exp.vpx.raw,1),1);
                    lost_track_full(st:ed)=lost_track;
                    
                    %Use median instead of mean as offsets can be off
                    %centered?
                    vpx.raw(~lost_track_full,2) = vpx.raw(~lost_track_full,2) - medfilt1(vpx.raw(~lost_track_full,2),600);
                    vpx.raw(~lost_track_full,3) = vpx.raw(~lost_track_full,3) - medfilt1(vpx.raw(~lost_track_full,3),6Z00);
        
                    if ip.Results.normalize
                        vpx.raw(~lost_track_full,2) = vpx.raw(~lost_track_full,2) / std(vpx.raw(~lost_track_full,2));
                        vpx.raw(~lost_track_full,3) = vpx.raw(~lost_track_full,3) / std(vpx.raw(~lost_track_full,3));
                    end
                    
                    lost_track_full2(st:ed)=lost_track;
                    
                    
                end
                vpx.raw(lost_track_full2==1,:)=[];
                %% Update if happy
                Exp.vpx=vpx;
                %Update calibration for zero centered
                nTrials = numel(Exp.D);
                for ii=1:nTrials
                    Exp.D{ii}.c=[0 0];
                    if ip.Results.normalize
...
function Exp = get_smo_eyetrace_filewise(Exp)


FileBreak=find(diff(Exp.vpx.raw(~isnan(Exp.vpx.raw(:,1)),1))>1);
