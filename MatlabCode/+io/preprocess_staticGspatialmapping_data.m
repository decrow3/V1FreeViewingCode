function [stimX, Robs, opts] = preprocess_staticspatialmapping_data(Exp, varargin)
...
if isempty(ip.Results.validTrials)
    validTrials = intersect(io.getValidTrials(Exp, 'StaticGabor'), io.getValidTrials(Exp, 'Ephys'));
...
mypars = cellfun(@(x) x.PR.hNoise.mypars, Exp.D(validTrials(:)), 'uni', 0);
...
xPosition = xpos - eyeAtFrame(:,1) - Exp.S.centerPix(1);
% yPosition = -ypos + eyeAtFrame(:,2);
yPosition = ypos - eyeAtFrame(:,2)- Exp.S.centerPix(1);
