function fig = checkCalibration1(Exp, eyePos, calmat)

% Check the existing gaze calibration
%% organize the data
% fprintf('Correcting eye pos by reanalyzing FaceCal\n')
% calmat of the form c (1x2), dx(1), dy(1) as in MarmoViewLastCalib
...
[~, id] = calibration_loss([calmat.c(1) calmat.c(2), 0 ,calmat.dx, calmat.dy], xy(ix,:), targets, std(xy(ix,1)));
